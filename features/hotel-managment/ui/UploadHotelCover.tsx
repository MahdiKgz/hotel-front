...
import { useUploadCoverMutation } from "@/entities/Hotel/services/hotel.service";

function UploadHotelCover({ slug }: { slug: string }) {
  // تغییر نوع fileList به File[] برای دسترسی مستقیم به فایل اصلی
  const [fileList, setFileList] = useState<File[]>([]);
  const [uploadCover, { isLoading }] = useUploadCoverMutation();
  ...
  const handleUpload = async () => {
    ...
    try {
      await uploadCover({ slug, file: previewFile }).unwrap();
      toast.success("کاور با موفقیت آپلود شد.");
      setFileList([]);
      return;
    } catch (error: any) {
      toast.error(error?.data?.message || "خطا در آپلود کاور.");
    }
  };
...
export default UploadHotelCover;
